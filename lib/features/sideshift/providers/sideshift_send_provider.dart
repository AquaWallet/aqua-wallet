...
final _pendingOrderStreamProvider =
  ...
  yield* ref.watch(sideshiftSendProvider)._pendingOrderSubject;
});
...
final sideshiftSendProvider = Provider(SideshiftOrderProvider.new);
...
class SideshiftOrderProvider {
  ...
  Future<void> _placeFixedRateOrder({
    required SideshiftAsset deliverAsset,
    required SideshiftAsset receiveAsset,
  ...
  Future<void> createOnchainTxForSwap(FeeAsset feeAsset, bool sendAll,
    ...
    switch (feeAsset) {
      ...
      case FeeAsset.tetherUsdt:
        await _createTaxiTxForSwap(sendAll, isLowball: isLowball);
      case FeeAsset.btc:
        assert(false, 'BTC fee asset not supported for sideshift');
    }
  }

  Future<void> _createTaxiTxForSwap(bool sendAll,
      {bool isLowball = true}) async {
    ...
    await _ref
        ...
        .executeTaxiTransaction(depositAddress, depositAmountPrecision, sendAll,
            isLowball: isLowball);
  }
