...
class SideswapWebsocketProvider {
  ...
  Future<void> _sendRequest(
    ...
    logger.debug('[Sideswap] Requesting $method: $params');
  ...
  Future<void> subscribeAsset(SubscribePriceStreamRequest request) async {
    logger.debug('[Sideswap] requesting: $request');
  ...
  void _onData(dynamic data) {
    ...
    logger.debug('[Sideswap] websocket reply: $json');

    if (json.containsKey('error')) {
      final error = Error.fromJson(json);
      logger.error('[Sideswap] $error');
      ref.read(swapLoadingIndicatorStateProvider.notifier).state =
      ...
      switch (method) {
        ...
        case swapDone:
          ...
          if (receiveAsset != null && transactionId != null && success) {
            ...
            // TODO Handle error
            logger.error('[Sideswap] Error: $json');
          }
          break;
        default:
          logger.warning('[Sideswap] unexpected response: $method');
          logger.warning('[Sideswap] ${json['result']}');
      }
    }
  }

  void _onError(dynamic error) {
    logger.error('[Sideswap] Error: $error');
  }

  void _onDone() {
    logger.debug('[Sideswap] Done');
    ref.read(sideswapWebsocketSubscribedAssetIdStateProvider.notifier).state =
        '';
    ref.read(swapLoadingIndicatorStateProvider.notifier).state =
        const SwapProgressState.empty();
